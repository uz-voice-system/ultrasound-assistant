...
public sealed class PatientCreatedEventHandler : IIntegrationEventHandler
{
    ...
    public PatientCreatedEventHandler(IPatientReadRepository repository)
    {
    ...
    {
        var @event = EventPayloadReader.Read<PatientCreatedEvent>(payload, nameof(PatientCreatedEvent));

        var patient = await _repository.GetByIdForUpdateAsync(@event.PatientId, cancellationToken);

        if (patient is not null && @event.Version <= patient.Version)
            return;
